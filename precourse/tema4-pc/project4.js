...
function obtainName(){
	var nameOfUser = prompt("Indica tu nombre de jugador:");
	if(nameOfUser == "" || nameOfUser == null){
		obtainName();
	}else{
		console.log("Hola " + nameOfUser + ". Prepárate para jugar!\n");
		return nameOfUser;
	}
}


function main(ranking = ""){

	if(ranking == ""){
		// el ranking no tiene valores, ergo es la primera partida
		ranking = [];
	}
	...
	console.log("Debes responder a las preguntas.\nSi completas una partida o más, se mostrará un ranking de puntuaciones.");
	console.log("Si no conoces la respuesta, puedes escribir 'pasapalabra' o dejarla en blanco para responderla más adelante.");
	console.log("Si quieres salir del juego, escribe 'END' y la partida terminará.");

	do {
		// recorro las preguntas:
		for(var i = 0; i <= questions.length -1; i++){

			//chequeamos si la pregunta ya ha sido respondida. Si es así, ya no la preguntamos más
			...
			if( (answer == null) || (answer == "pasapalabra") || (answer == "") ){
				console.log("Ok, saltamos a la siguiente pregunta!\n");
				numberOfTurns++;
			}else if(answer == "END"){
				console.log("Lamentamos que hayas decidido dejar de jugar. Tu puntuación era de: " + hits + " aciertos.\n");
				playing = false;
				break;
		...
		if( (hits + errors) == 27 ){
			// todas las preguntas han sido respondidas, ya no preguntaré nada más.
			playing = false; // para que podamos salir del bucle


			// inserto la puntuación en el ranking ya existente:
			var newRankingData = {
				name: nameOfUser,
				hits: hits
			}
	...
	// aquí debo mostrar el ranking:
	if(ranking.length != 0){
		console.log("-----------------\nEste es el ranking de puntuaciones:");
		...
		for (var i = ranking.length - 1; i >= 0; i--) {
			console.log(ranking[i].name + " tiene una puntuación de: " + ranking[i].hits + " aciertos!");
		}
		console.log("-----------------\n");
	}

	if(confirm("¿Nueva partida?")){
		// otro jugador va a jugar
		main(ranking);
	}
